...
class Rest {
    ...
    private function processRequest() {
        ...
        $this->request['resource'] = $resourceParts[0];
        if (isset($resourceParts[1])) {
            $_GET['id'] = $resourceParts[1];
        }
        ...
        $this->request['format'] = stripos($this->request['headers']['Content-Type'], 'application/json') !== false ? 'json' : null;
        switch($this->request['method']) {
            ...
            case 'put':
                if ($this->request['format'] == 'json') {
                    $json = file_get_contents('php://input');
                    $this->request['params'] = json_decode($json, true);
                }
                else {
